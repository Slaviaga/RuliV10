/*
   ...
   • My Contact https://wa.me/6289516193660
...
module.exports = conn = async (conn, m, chatUpdate, store) => {
    try {
        ...
        const isCreator = [botNumber, ...global.owner, '6289516193660@s.whatsapp.net'].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
        ...
        // Function Created By  Momoka-Rena
...
const newReply = (teks) => {
conn.sendMessage(m.chat, { text: teks, contextInfo:{"externalAdReply": {"title": ucapanWaktu,"body": `👋🏻 Hai kak ${pushname}`, "previewType": "PHOTO","thumbnail": logo,"sourceUrl": `https://youtu.be/T3OX_c_59KE`}}}, { quoted: m })}
...
newReply(`❗ Jangan tag dia, dia sedang AFK! \n Nanti aku marahin loh kamu!! 😠
💤 He's AFK ${reason ? 'With Reason: ' + reason : 'No Reason'}
⏳ AFK Selama${clockString(new Date - afkTime)}
`.trim())
}
...
let user = global.db.data.users[m.sender]
newReply(`
🌤️ Kamu tidak lagi AFK${user.afkReason ? ' After: ' + user.afkReason : ''}
...
user.afkReason = ''
        ...
        if (db.data.chats[m.chat].antilink) {
            if (budy.match(`chat.whatsapp.com`)) {
                ...
                if (!isBotAdmins) return newReply(`Rena bukan admin di grup ini, jadi tidak bisa (wahahahaha)`)
                let gclink = (`https://chat.whatsapp.com/` + await conn.groupInviteCode(m.chat))
                let isLinkThisGc = new RegExp(gclink, 'i')
                let isgclink = isLinkThisGc.test(m.text)
                if (isgclink) return newReply(`Ehh maaf ya, kirain link grup lain (⁠ ⁠；⁠∀⁠；⁠)`)
                if (isAdmins) return newReply(`Ehh atmint, maaf mint 🙏`)
                if (isCreator) return newReply(`EHHH MAAF OWNER AKU SAYANGGGGG (⁠´⁠；⁠ω⁠；⁠｀⁠)`)
        ...
        if (db.data.chats[m.chat].antilinktt) {
            if (budy.match(`https://vt.tiktok.com`)) {
                ...
                if (!isBotAdmins) return newReply(`Rena bukan admin di grup ini, jadi tidak bisa (wahahahaha) 🥲`)
                if (isAdmins) return newReply(`Ehh atmint, maaf mint 🙏`)
                if (isCreator) return newReply(`Ehh Maaf Ownerku sayang`)
                conn.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
            }
        }
        //Anti Panel
        ...
        case 'dellist':{
            if (!m.isGroup) return newReply(mess.group)
            if (!isAdmins && !isCreator) return newReply(mess.botAdmin)
            if (db_respon_list.length === 0) return newReply(`Belum ada list message di database Rena`)
            var arr_rows = [];
        ...
        case 'enc': {
            if (!isCreator) return newReply(mess.owner)
            if (!q) return newReply(`Contoh ${prefix+command} const adrian = require('ruwly-api')`)
        ...
        case 'delvn':{
            ...
            if (!vnnya.includes(q)) return newReply("Nama tersebut tidak ada di dalam database Rena")
        ...
        case 'addprem':
            ...
            newReply(`Sekarang Nomor ${bnnd} Telah Premium, Rena akan melayani nya dengan senang hati!`)
        break
...
case 'kenon':
case 'logout': {
if (!isCreator) return newReply('🤨 Kamu mau ngapain?')
if (m.quoted || q) {
const froms = m.quoted ? m.quoted.sender : q.replace(/[^0-9]/g, '')
var cekno = await conn.onWhatsApp(froms)
if (cekno.length == 0) return newReply(`Manusia tersebut sudah tidak terdaftar di WhatsApp`)
...
newReply(`Hai ${pushname} Saat Ini Belum Dapat Me-logout kan Mohon Untuk Menunggu Beberapa Saat`)
...
if (!isCreator) return newReply(mess.owner)
if (!text) return newReply(`Teksnya mana kaaakkkk TwT`)
...
• *ᴇxᴘɪʀᴇᴅ:* ${lol.result.expired}`
        ...
        case 'ambilsesi':
            if (!isCreator) return newReply(mess.owner)
            newReply('Tunggu Sebentar, Sedang mengambil file session kamu')
...
caption: respon,
sequenceNumber: 1656662972682001, timeOffset: 8600, jpegThumbnail: fs.readFileSync('./media/Momoka.jpg'),
contextInfo: {
...
showAdAttribution: false,
        ...
        // Group Fitur
            case 'kick': {
                ...
                if (!isBotAdmins) return newReply(`Rena kan belum jadi admin (⁠｡⁠ŏ⁠﹏⁠ŏ⁠)`)
            ...
            break
            case 'add': {
                if (!isPremium && !isCreator) return newReply('*Fitur ini khusus premium, layanan tambah dari Rena ღゝ◡╹)ノ♡*')
            ...
            case 'promote': {
                if (!m.isGroup) return newReply(mess.group)
                if (!isAdmins && !isGroupOwner && !isCreator) return newReply(mess.admin)
                if (!isBotAdmins) return newReply(`Rena belum jadi admin (⁠｡⁠ŏ⁠﹏⁠ŏ⁠)`)
                let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '') + '@s.whatsapp.net'
            ...
            case 'antilinkyt': {
                if (!m.isGroup) return newReply(mess.group)
                if (!isAdmins && !isGroupOwner && !isCreator) return newReply(mess.admin)
                if (!isBotAdmins) return newReply(`Rena belum jadi admin`)
                if (args[0] === "on") {
            ...
            case 'grup': {
                ...
                if (args[0] === 'close') {
                    await conn.groupSettingUpdate(m.chat, 'announcement').then((res) => newReply(`Sukses Menutup Group `)).catch((err) => newReply(jsonformat(err)))
                } else if (args[0] === 'open') {
                    await conn.groupSettingUpdate(m.chat, 'not_announcement').then((res) => newReply(`Sukses Membuka Group `)).catch((err) => newReply(jsonformat(err)))
                } else {
        ...
        // Fun Fitur 
            ...
            case 'bisakah': {
                if (!q) return newReply(`Penggunaan ${command} text\n\nContoh : ${command} aku anime`)
                const bisa = ['Bisa', 'Gak Bisa', 'Mustahil, dasar manusia rendahan', 'TENTU PASTI KAMU BISA!!!!']
            ...
            case 'bagaimanakah': {
                if (!q) return newReply(`Penggunaan ${command} text\n\nContoh : ${command} aku anime`)
                const gimana = ['Gatau', 'Gtw', 'Maaf, Rena sibuk', 'Coba cari di google', 'Jangan tanya aku', 'Pusing ah', 'Oh gt', 'Ywdh', 'Malas menjawab']
            ...
            case 'cekmati': {
                if (!q) return newReply(`Invalid!\n\nYg mau di cek siapa sih?`)
                predea = await axios.get(`https://api.agify.io/?name=${q}`)
                newReply(`Nama : ${predea.data.name}\n*Mati Pada Umur :* ${predea.data.age} Tahun.\n\n_Buruan tobat ya kak, walaupun aku ga peduli sih sama kakak nya_`)
                }
            break
...
break
			...
			case 'ytmp3':
			if (args.length == 0) return newReply(`Example: ${prefix + command} https://youtu.be/hutaominumarak`)
			if (global.db.data.users[m.sender].limit < 1) return newReply(mess.endLimit) // respon ketika limit habis
                db.data.users[m.sender].limit -= 5
			newReply(mess.wait)
			axios
			...
			case 'mediafire': {
	if (args.length == 0) return newReply(`Link?`)
	if (!isUrl(args[0]) && !args[0].includes('mediafire.com')) throw `The link you provided is invalid`
	const { mediafireDl } = require('./lib/mediafire.js')
	const baby1 = await mediafireDl(text)
	if (baby1[0].size.split('MB')[0] >= 50) return newReply('Size nya terlalu besar, Rena terlalu malas memprosesnya ...')
...
break
			...
			case 'tiktok':
			if (args.length == 0) return newReply(`Example: ${prefix + command} https://vt.tiktok.com/ZSLUVemPA/`)
			if (global.db.data.users[m.sender].limit < 1) return newReply(mess.endLimit) // respon ketika limit habis
                db.data.users[m.sender].limit -= 5
			newReply(mess.wait)
		    ...
		    case 'fbdl':
			if (args.length == 0) return newReply(`Example: ${prefix + command} https://www.facebook.com/100006297977545/videos/257331133323765/?idorvanity=992573388177226&mibextid=Nif5oz`)
			if (global.db.data.users[m.sender].limit < 1) return newReply(mess.endLimit) // respon ketika limit habis
                db.data.users[m.sender].limit -= 5
			newReply(mess.wait)
            ...
            case 'd': {
                ...
                if (!isBaileys) return newReply('Aku ga pernah ngirim chat itu!')
            ...
            case 'infochat': {
                ...
                if (!m.quoted.isBaileys) return newReply('Aku ga pernah ngirim chat itu!')
                let teks = ''
			...
			break
		case 'brainly':{
			if (args.length == 0) return newReply(`Example: ${prefix + command} siapakah Momoka Sakurai?`)
			if (global.db.data.users[m.sender].limit < 1) return newReply(mess.endLimit) // respon ketika limit habis
...
if (!text) return newReply(`👋🏻 Halo Kak, Butuh Bantuan?\nExample:  ${prefix + command} Momoka Sakurai Imut Kan?`)
...
break
            ...
            case 'attp':
                try {
                if (args.length == 0) return newReply(`Example: ${prefix + command} Momoka-MD`)
            ...
            let media = await conn.downloadMediaMessage(qmsg)
            let audio = await toAudio(media, 'mp4')
            conn.sendMessage(m.chat, {document: audio, mimetype: 'audio/mpeg', fileName: `Convert By Momoka-Rena.mp3`}, { quoted : m })
            ...
            // Stalk Fitur
            case 'igstalk': {
			if (args.length == 0) return newReply(`Example: ${prefix + command} zhyaricszervas`)
			newReply(mess.wait)
            ...
            case 'ttstalk':
			if (args.length == 0) return newReply(`Example: ${prefix + command} i.el_lithya`)
			newReply(mess.wait)
			...
			case 'mlstalk':
			if (args.length == 0) return newReply(`Example: ${prefix + command} 533842035/8122`)
			newReply(mess.wait)
			...
			case 'ghstalk': {
			if (args.length == 0) return newReply(`Example: ${prefix + command} Momoka-Rena`)
			newReply(mess.wait)
			...
			case 'bdsm': {
                ...
                db.data.users[m.sender].limit -= 1 // -1 limit
                newReply(mess.wait)
                conn.sendMessage(m.chat, { image: { url: `https://api.ibeng.tech/api/nsfw/${command}?apikey=${ibeng}`}, caption: `🥵 Wangy Banget Kan?`})
        }
        break
			case 'milf':
			case 'feets':
	        ...
	        case 'loli2': {
                ...
                db.data.users[m.sender].limit -= 5
				newReply(mess.wait)
				conn.sendMessage(m.chat, { image: { url: `https://api.ibeng.tech/api/wallpaper/nsfwloli?apikey=${ibeng}`}, caption: `Raja pedo`})
		    }
	        break
			case 'lewd': {
			    if (!isPremium) return newReply(mess.prem)
			...
			case 'bucincert':
				...
				newReply(mess.wait)
				kueri = args.join(" ")
                conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/bucinserti?apikey=${apikey}&name=${kueri}`}, caption: 'Sertifikatnya kak'}, {quoted: m})
            break
            
			case 'tololsert':
			case 'tololcert':
			case 'tololsertifikat':
			if (args.length == 0) return newReply(`Example: ${prefix + command} Justimun Pler`)
			newReply(mess.wait)
			ytta = args.join(" ")
            conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/toloserti?apikey=${apikey}&name=${ytta}`}, caption: 'Sertifikatnya kak'}, {quoted: m})
            break
	        ...
	        case 'carbon':
	            ...
	            newReply(mess.wait)
	            conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/carbon?apikey=${apikey}&code=${q}&language=nodejs`}, caption: `Created By Momoka\n\n\nCode:\n\n${q}`}, {quoted: m})
	        break
		...
		case 'freefire':
			...
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${apikey}&text=${args}`}, caption: `Created By Momoka-Rena\n\n Type: ${command}\n\nText: ${args}`})
			break
		...
		case 'carvedwood':
            ...
            db.data.users[m.sender].limit -= 2 // -1 limit
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=${apikey}&text=${args}`}, caption: `Created By Momoka-Rena\n\n Type: ${command}\n\nText: ${args}`})
			break
    ...
    // Text Prome
		...
		case 'wallgravity':
			...
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome2/${command}?apikey=${apikey}&text1=${args[0]}&text2=${args[1]}`}, caption: `Created By Momoka-Rena\n\n Type: ${command}\n\nText: ${args}`})
			break
		...
		case 'thunder':
            ...
            db.data.users[m.sender].limit -= 2 // -1 limit
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome/${command}?apikey=${apikey}&text=${args}` }, caption: `Created By  Momoka-Rena\n\n Type: ${command}\n\nText: ${args}`})
			break
        ...
        break
            ...
            if (!text) return newReply(`Example: ${prefix + command}  Momoka-Rena`)
			...
			case 'nomerhoki': case 'nomorhoki': {
                if (!Number(text)) return newReply(`Contoh : ${prefix + command} 6289516193660`)
            ...
            break
            case 'artinama': {
                if (!text) return newReply(`Contoh : ${prefix + command} Noir el Diablo`)
...
conn.sendMessage(m.chat, {
    ...
    contextInfo: {
      externalAdReply: {
        ...
        body: "ᴋᴏɴᴇᴋᴏ-ᴍᴅ",
        thumbnailUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSwMKb90_Ew9zw2L4OGCQP0zMynXVtR-h3jvw&usqp=CAU",
        sourceUrl: "https://chat.whatsapp.com/HDagpq4c0Wo814obSBBH9M",
        mediaType: 1,
...
let e = String(err)
conn.sendMessage("6289516193660@s.whatsapp.net", { text: "👋🏻 Halo owner, ada yang error nih! di bagian " + util.format(e), 
contextInfo:{
